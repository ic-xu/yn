...
const PlantumlExtendPlugin = (md: Markdown) => {
  ...
  md.renderer.rules.fence = (tokens, idx, options, env, slf) => {
    ...
    if (token.info === 'plantuml') {
      ...
      /// content 是plantuml 里面的内容
      const imgSrc = generateSourceDefault(content)
      token.attrs = [
        ['src', imgSrc],
        ['alt', '']
      ]
      // token[ids].attrs
      return render(tokens, idx) as any
    }
...
export default {
  name: 'extension-plantuml-ext',
  register: ctx => {
    ctx.markdown.registerPlugin(PlantumlExtendPlugin)
